...
func (m *MockPubSubService) SendOncallerAcknowledgedMessage(ctx context.Context, incidentID string, serviceID uint64, onCaller string) error {
	args := m.Called(ctx, incidentID, serviceID, onCaller)
	return args.Error(0)
}
